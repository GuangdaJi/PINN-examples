...
def train(
    ...
    for i in range(epoch):
        ...
        t = torch.rand(size=(batch_size, 1), dtype=torch.float, device=device, requires_grad=True)
        ...
        for j in range(step):
            ...
            IC = model.initial_condition(theta_10, theta_20, t_init, init_params)
            ...
            print(
                'epoch/step:{:04d}/{:04d}, EoM: {:.08e}, IC: {:.08e}, loss: {:.08e}'.format(
                    i, j, mse_eom.item(), mse_ic.item(), loss.item()
                )
            )
    ...
    txy_1 = torch.cat([t, xy_1], dim=1).numpy()
    txy_2 = torch.cat([t, xy_2], dim=1).numpy()
